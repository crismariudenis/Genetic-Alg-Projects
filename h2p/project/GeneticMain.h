...
public:
    ...
    void computeFitness()
    {
        ...
        for (auto &p : pop)
        {
            p.fitness = std::pow(((maxFitness - p.fitness) / (maxFitness - minFitness + eps) + 1), POWER);
            // p.fitness = 1.0 / (p.fitness + eps);
            sum += p.fitness;
        }
    ...
    std::pair<double, double> run(int generations)
    {
        ...
        for (int g = 0; g < generations; g++)
        {
            ...
            if (!useGrayCode && g >= generations * GEN)
            {
                for (auto &p : pop)
                {
            ...
            for (int i = pop.size() * elitism; i < pop.size(); i++)
            {
